...
func main() {
	...
	conn, err := sql.Open("pgx", "host=localhost port=5432 dbname=test_connect user=postgres password=********")
	if err != nil {
	...
	// update a row
	stmt := `update users set id = $1 where first_name= $2`
	_, err = conn.Exec(stmt, 5, "Jackie")
	if err != nil {
		log.Fatal(err)
	}
